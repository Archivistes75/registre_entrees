...
data <- eventReactive(input$file1,{
   data <- read_csv(input$file1$datapath)  # import browse
})

#data <- eventReactive(input$file1,{
 #  data <- read_csv(input$text1)      # import lien 
#})
```
...
```{r, echo = FALSE}
fileInput("file1", "Veuillez choisir le fichier des données au format CSV.",
                    multiple = TRUE,
                    accept = c("text/csv",
                             "text/comma-separated-values,text/plain",
                             ".csv"))

#textinput("text1", "Veuillez entrer le lien des données", value="")
```


```{r include=FALSE}
data <- read_csv("Avignon_standardise.csv")
# Format des données
str(data)
...
data$modeEntree <- str_replace_all(data$modeEntree, c("Collecte"="Collecte privée"))
```
...
t1 <- as.data.frame(table(data$dateYear))   # ces commandes permettent de récupérer des fréquences pour toutes les années même si aucune archive n'a été rentrée (:freq=0 alors)
...
```{r}
  # Création de valeurs random pour naturesupport car champ obligatoire ds le standard mais pas renseigné dans jeu d'essai Avignon
nature <- c("Support physique", "Support électronique", "Support mixte")
data$natureSupport <- sample(nature, size = nrow(data), replace = TRUE)
