...
Le standard {data-icon="fa-clipboard-list"}
=====================================  
...
### Présentation du schema {data-padding=35}
...
Les **colonnes non obligatoires** qui composent le standard :
...
En quelques mois, une équipe composée de représentants du SIAF, de l'AAF et de Dactactivist ont travaillé pour mettre au point un standard pour les données des **registres d'entrées**. Ce dernier n'a pas de caractère réglementaire mais son utilisation est recommandée dès lors qu'une réutilisation ou une comparaison des données est envisagée.
...
```{r out.width='97%', fig.align='left', echo=FALSE}
...
```{r echo=FALSE, out.width='100%', fig.align='bottom'}
wordcloud2(data=dat2, size=2, col=rep_len(c("#66CCFF", "#0099CC", "#3399CC", "#006699", "#0066FF", "#0033CC", "#0066CC", "#0000FF"), nrow(dat2)), minRotation = -pi/6, maxRotation = -pi/6, rotateRatio = 0)
```
