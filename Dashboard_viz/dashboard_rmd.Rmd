...
```{r echo=FALSE}
valueBox("36%", caption="Archives publiques", color = "#FFCC00", icon = "ion-loop")
...
```{r echo=FALSE}
valueBox(853, caption="Séries", color = "#0066CC", icon = "ion-filing")
```
...
plot2_t2 <- t2_bis %>% 
  ...
  theme(axis.text.y = element_text(size = rel(.8)),
        ...
        legend.key = element_rect(fill = "white", colour = NA), complete = TRUE) #reproduction du theme_bw avec lequel il n'est pas possible de changer la taille des modes d'entrées : axis.text.y
...
### Nombre de documents entrés chaque année selon le type {.no-mobile}
...
texte pour expliquer le standard, démarches, collaborations etc.
...
### Wordcloud 2 (même mais avec colonne descContenu et couleurs différentes)
...
```{r echo=FALSE}
wordcloud2(data=dat2, size=1.6, backgroundColor="#FAF3EE", color=rep_len(c("#1D3139","#E5555C"), nrow(dat2)), minRotation = -pi/6, maxRotation = -pi/6, rotateRatio = 1)
```

### descContenu en letterCloud SIAF

```{r}
wordcloud2::letterCloud(data=dat2, size=5, "SIAF", backgroundColor="#FAF3EE", color=rep_len(c("#1D3139","#E5555C"), nrow(dat2))) #censé rendre "Wordclou_siaf.png"
```
